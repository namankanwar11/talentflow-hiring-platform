...
export function AssessmentBuilderPage() {
  ...
  const addQuestion = () => { setQuestions([...questions, { id: Date.now(), type: 'short-text', question: '', validation: { required: false } }]); };
  ...
  return (
    <div className="max-w-7xl mx-auto">
        <div className="flex items-center justify-between mb-8">
            <div>
                <h2 className="text-3xl font-bold tracking-tight text-gray-900">Assessment Builder</h2>
                <Link to="/jobs" className="text-sm text-gray-500 hover:underline">&larr; Back to Jobs</Link>
            </div>
            <div className="flex items-center gap-4">
                <Link to={`/jobs/${jobId}/assessment/take`} className="px-4 py-2 text-sm font-semibold text-gray-700 bg-white border border-gray-300 rounded-lg shadow-sm hover:bg-gray-50">
                ...
                <div className="space-y-4">
                    {isLoading ? <p>Loading...</p> : questions.map((q, qIndex) => (
                        <div key={q.id} className="p-4 border rounded-md bg-gray-50/80 space-y-3">
                            ...
                            <select value={q.type} onChange={(e) => updateQuestion(qIndex, { type: e.target.value as QuestionType, options: (e.target.value === 'single-choice' || e.target.value === 'multi-choice') ? ['New Option'] : undefined })} className="w-full p-2 border border-gray-300 rounded-md bg-white">
                ...
                <div className="space-y-6">
                    {questions.length === 0 ? <p className="text-center text-gray-500 py-8">Your form preview will appear here.</p> : questions.map((q, index) => (
                        <div key={q.id}>
                            ...
                            <div className="mt-2">
                                {q.type === 'short-text' && <input type="text" className="w-full p-2 border border-gray-300 rounded-md bg-gray-100" disabled />}
                                {q.type === 'long-text' && <textarea className="w-full p-2 border border-gray-300 rounded-md bg-gray-100" rows={4} disabled></textarea>}
                                {q.type === 'numeric' && <input type="number" className="w-full p-2 border border-gray-300 rounded-md bg-gray-100" disabled />}
                                {q.type === 'file-upload' && <div className="text-sm p-4 text-center border-2 border-dashed rounded-lg text-gray-500 bg-gray-100">File Upload Placeholder</div>}
